...
def Survey3_Convert_RAW_to_Tiff(infolder, outfolder, wb):
    ...
    if numFiles > 1:
        first_files_jpg = (
            "JPG" in infiles[0].upper() and "JPG" in infiles[1].upper()
        )  # returns true if first two files are JPG
        first_files_rawjpg = (
            "RAW" in infiles[0].upper() and "JPG" in infiles[1].upper()
        )  # returns true if first two files are RAW and JPG
        if first_files_jpg or first_files_rawjpg:
            ...
            for file in files_to_process:
                if first_files_rawjpg:
                    try:
                        print(
                            f"Processing file {count}/{len(files_to_process)} {file[2:]}"
                        )
                        ...
                        udata = np.packbits(
                            np.concatenate(
                                [
                                    data[0::3],
                                    np.array(
                                        [0, 0, 0, 0] * 12000000, dtype=np.uint8
                                    ).reshape(12000000, 4),
                                    data[2::3],
                                    data[1::3],
                                ],
                    ...
                    try:
                        color = cv2.cvtColor(img, cv2.COLOR_BAYER_RG2RGB).astype(
                            "float32"
                        )
                    except Exception as e:
                        exc_type, exc_obj, exc_tb = sys.exc_info()
...
def color_correction(color):
    ...
    color[:, :, 2] = (
        (red_coeffs[0] * color[:, :, 0])
        + (red_coeffs[1] * color[:, :, 1])
        + (red_coeffs[2] * color[:, :, 2])
        + roff
    )
    color[:, :, 1] = (
        (green_coeffs[0] * color[:, :, 0])
        + (green_coeffs[1] * color[:, :, 1])
        + (green_coeffs[2] * color[:, :, 2])
        + goff
    )
    color[:, :, 0] = (
        (blue_coeffs[0] * color[:, :, 0])
        + (blue_coeffs[1] * color[:, :, 1])
        + (blue_coeffs[2] * color[:, :, 2])
        + boff
    )
